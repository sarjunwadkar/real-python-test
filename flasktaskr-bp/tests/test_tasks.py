...
class TasksTest(unittest.TestCase):
  ...
  def create_admin_user(self):
    new_user = User(name='Superman', email='admin@gmail.com', password='allpowerful', role='admin')
    db.session.add(new_user)
    db.session.commit()

  def tests_logged_in_users_can_access_tasks_page(self):
  ...
  def test_users_cannon_complete_tasks_that_are_not_created_by_them(self):
    self.create_user('Michael', 'michael@gmail.com', 'python')
    self.login('Michael', 'python')
    self.app.get('tasks/', follow_redirects=True)
    self.create_task()
    ...
    self.create_user('Fletcher', 'fletcher@gmail.com', 'python')
  ...
  def test_string_reprsentation_of_the_task_object(self):
    ...
    db.session.add(
            Task("Run around in circles", date(2015, 1, 22),
                10, date(2015, 1, 23), 1, 1)      )
    db.session.commit()
